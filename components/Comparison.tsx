...
export const Comparison: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-white relative overflow-hidden border-t border-white/5">
      {/* Background Effects */}
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <div className="text-center mb-12">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-gray-200 mb-4"
          >
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-gray-50/50 backdrop-blur-sm border border-gray-200 rounded-2xl overflow-hidden"
        >
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left p-4 text-gray-600 font-medium text-sm">Feature</th>
                  {alternatives.map((alt, idx) => (
                    <th
                      key={idx}
                      className={`text-center p-4 font-semibold text-sm ${
                        alt.highlight
                          ? 'bg-accent/10 text-purple-700 border-x border-accent/20'
                          : 'text-gray-700'
                      }`}
                    >
                      <div className="flex flex-col items-center gap-1">
                        {alt.highlight && (
                          <div className="px-2 py-0.5 bg-accent rounded-full text-[10px] font-bold text-gray-900 mb-1">
                            BEST VALUE
                          </div>
                        )}
                        <div>{alt.name}</div>
              ...
              <tbody>
                {features.map((feature, featureIdx) => (
                  ...
                  >
                    <td className="p-4 text-gray-700 text-sm font-medium">{feature}</td>
        ...
        >
          <div className="inline-flex flex-col sm:flex-row items-center gap-6 bg-gray-50/50 border border-gray-200 rounded-2xl p-6">
            ...
            <button
              ...
              className="px-6 py-3 bg-accent hover:bg-accent-dark text-gray-900 rounded-lg font-bold transition-all hover:shadow-lg hover:shadow-accent/20 inline-flex items-center gap-2 whitespace-nowrap"
            >
              See It in Action
              <TrendingUp size={16} />
            </button>
